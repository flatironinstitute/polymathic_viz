...
def clean_unprocessed():
    if os.path.exists(unprocessed_dir) and os.path.isdir(unprocessed_dir):
        # Remove everything in the directory
        shutil.rmtree(unprocessed_dir)
        # Optionally recreate the empty directory
        os.makedirs(unprocessed_dir)
        print(f"All contents of {unprocessed_dir} have been removed.")
    else:
        print(f"{unprocessed_dir} does not exist or is not a directory.")
...
class collage_maker:
    ...
    def save(self, to_dir=output_dir, save_as="final.mp4", fps=7):
        ...
        save_video(self.biggif, f"{to_dir}/{save_as}", fps)
